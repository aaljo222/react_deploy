...
const ProductCategoryFashion = () => {
  ...
  const category = "Fashion"; // Category filter
  const itemsPerPage = 4; // Number of items to display at once

  useEffect(() => {
    // Fetch products for the Fashion category
    const fetchProducts = async () => {
      ...
      try {
        ...
        setProducts(response.dtoList || []); // Adjust according to API's response structure
      } catch (error) {
